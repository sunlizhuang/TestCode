...
class NWPURESISCDataset(data.Dataset):
    def __init__(self, root, train=True, transform=None, split=None, tag=None):
        ...
        print('Creating dataset with {} examples'.format(len(self.targets)))
...
class FAKEDataset(data.Dataset):
    def __init__(self, root, train=True, transform=None, split=None, tag=None):
        ...
        for item in train_infos:
            ...
            # print(item)
            fname,idx=item.replace("\n","").split(" ")
            trn_files.append(os.path.join(root + '/image', fname))
            trn_targets.append(int(idx))
    ...
    def __getitem__(self, i):
        ...
        return img, self.targets[i]
class JL1Dataset(data.Dataset):
    def __init__(self, root, train=True, transform=None):
        train_infos=os.listdir(root)
        trn_files = []
        trn_targets = []
        for item in train_infos:
            fname=item.replace("\n","")
            trn_files.append(os.path.join(root, fname))
    ...
    def __getitem__(self, i):
        ...
        return img, self.targets[i]
import re
class FAKETestDataset(data.Dataset):
    def __init__(self,args, root, train=False, transform=None):
        train_infos=os.listdir(root)
        #train_infos=sorted(train_infos,key=lambda x: int(re.search(r'image-(\d+).png',x).group(1)))
        trn_files = []
        trn_targets = []
        for item in train_infos:
            fname=item.replace("\n","")
    ...
    def __getitem__(self, i):
        ...
        return img, self.targets[i],self.file_name[i]
...
def build_dataset(is_train, args):
    ...
    if args.dataset == 'imagenet':
        ...
        data_path = '../Dataset/fake'
        args.nb_classes = 2
        dataset = FAKEDataset(data_path, train=is_train, transform=transform, split=args.split, tag=args.tag)
    elif args.dataset == 'faketest':
        print('Loading fake dataset!')
        data_path = args.data_path
        args.nb_classes = 2
        dataset = FAKETestDataset(args,data_path, train=is_train, transform=transform)
    else:
